...
with open("D:/gliph-1.0/gliph/bin/Epitope_DATA.txt") as b:
  EPs = b.readlines()
b.close()
...
for e in edge_data:     #for every source and target after gliph
    ...
    if src in cl18 and dst in cl18: #check if source and data are both present in convergencegroup (18)
        for i in range(0,x):   #itterate over 0-35 for every HLA in HLAs
            if src in EPs[i]:  #if source in one of the HLA lists..
                klr_net.add_node(src, src, title=src)   #add source node to prob_net with color assigned to i
        for i in range(0,x):   #itterate over 0-35 for every HLA in HLAs
            if dst in EPs[i]:  #if target in one of the HLA lists..
...
for point in klr_net.nodes: #for every node
    for i in range(0,x): # create a loop over 35 numbers(enough to cover all HLAs in this case)
        if point["id"] in EPs[i]: #if node sequence in the list of HLA[i]
            name = EPs[i].split()[0] #assign the HLA-type to name
...
for e in edge_data:     #for every source and target after gliph
    ...
    if src in cl18 and dst in cl18: #check if source and data are both present in convergencegroup (18)
        for i in range(0,x):   #itterate over 0-35 for every HLA in HLAs
            if src in EPs[i]:  #if source in one of the HLA lists..
                name = EPs[i].split()[0] #get the HLA-type of this source
        ...
        for i in range(0,x):   #itterate over 0-35 for every HLA in HLAs
            if dst in EPs[i]:  #if target in one of the HLA lists..
                name = EPs[i].split()[0] #get the HLA-type of this target
...
htmll = htmls+legenda
htmlf = htmll + html[-15:]
H= open("D:/gliph-1.0/gliph/bin/TCRL457EP.html","w+")
H.write(htmlf)
H.close()
